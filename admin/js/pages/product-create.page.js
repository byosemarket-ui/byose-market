(async function () {
	...
	try {
		...
		status.textContent = error?.message || 'Unable to load the centralized catalog right now.';
	...
	const detailsSections = document.getElementById('productDetailsSections');
	const detailsStepBanner = document.getElementById('detailsStepBanner');
	const detailsStepText = document.getElementById('detailsStepText');
	const openCreatedProductButton = document.getElementById('openCreatedProductButton');
	const categorySelect = document.getElementById('productCategorySelect');
	const visibilitySelect = document.getElementById('productVisibilitySelect');
	const prioritySelect = document.getElementById('productPrioritySelect');
	const orderIndexInput = document.getElementById('productOrderIndexInput');
	const highlightTagSelect = document.getElementById('productHighlightTagSelect');
	...
	const state = {
		...
		mainImage: '',
		gallery: [],
		attributes: service.createAttributeTemplate(categorySelect.value),
		previousCategory: String(categorySelect.value || 'general')
	};
	...
	subtitle.textContent = 'Create the basic product first, then unlock and save the full details page content.';
	createSaveButton.textContent = 'Create Product';
	...
	function getVisibilityLabel(value) {
		...
		return 'Home and Shop';
	}

	function getPriorityLabel(value) {
		return value === 'top' ? 'Top priority' : 'Normal priority';
	}
	...
	function buildPayload() {
		...
		const isDetailsStage = Boolean(state.productId);
		const gallery = isDetailsStage
			? service.uniqueStrings([state.mainImage || raw.mainImage, ...state.gallery])
			: service.uniqueStrings([state.mainImage || raw.mainImage]);
		const attributes = isDetailsStage ? buildCleanAttributesForSave(state.attributes) : [];
	...
	function renderPreview() {
		...
		const oldPriceMarkup = Number(payload.oldPrice || 0) > Number(payload.price || 0)
			...
			: '';
		const shortDescription = payload.shortDescription || 'This product preview updates live while you prepare the storefront card.';
		const visibilityLabel = getVisibilityLabel(payload.visibility);
		const priorityLabel = getPriorityLabel(payload.priority);
		const highlightLabel = getHighlightTagLabel(payload.highlightTag);

		preview.innerHTML = `
			<div class="product-preview-stack">
				<section class="product-preview-card">
					<img src="${service.escapeHtml(displayImage)}" alt="${service.escapeHtml(payload.name || 'Product preview')}">
					<div>
				...
				<section class="product-preview-card product-preview-card--detail">
					<div>
						<p class="product-preview-eyebrow">Details Page Status</p>
						<h3>${state.productId ? `Product ID ${state.productId}` : 'Create product to unlock full details'}</h3>
						<ul class="product-preview-detail-list">
							<li><span>Visibility</span><strong>${service.escapeHtml(visibilityLabel)}</strong></li>
							<li><span>Priority</span><strong>${service.escapeHtml(priorityLabel)}</strong></li>
							<li><span>Order index</span><strong>${Number(payload.orderIndex || 0)}</strong></li>
							<li><span>Highlight tag</span><strong>${service.escapeHtml(highlightLabel)}</strong></li>
	...
	function syncStateFromProduct(product) {
		...
		const savedMainImage = String(product.mainImage || product.image || state.mainImage || '').trim();
		state.mainImage = savedMainImage;
		mainImageInput.value = savedMainImage;
		state.gallery = service.uniqueStrings(
			(Array.isArray(product.gallery) ? product.gallery : []).filter((image) => String(image || '').trim() && String(image || '').trim() !== savedMainImage)
		);
		state.attributes = Array.isArray(product.attributes) && product.attributes.length
			? normalizeEditableAttributes(product.attributes)
			: [];
		...
		highlightsRepeater.setValues(Array.isArray(product.highlights) ? product.highlights : []);
		trustRepeater.setValues(Array.isArray(product.trust) ? product.trust : []);
		specsRepeater.setValues(Array.isArray(product.specs) ? product.specs : []);
		refreshEverything();
	}

	function unlockDetails(product) {
		state.productId = Number(product.id);
		detailsSections.hidden = false;
		detailsStepBanner.hidden = false;
		openCreatedProductButton.hidden = false;
		openCreatedProductButton.href = `view.html?id=${encodeURIComponent(product.id)}`;
		createSaveButton.textContent = 'Save Product Details';
		detailsStepText.textContent = `${product.name} is now in the shared catalog. Continue editing the full details below and save again.`;
		title.textContent = `Continue ${product.name}`;
		subtitle.textContent = `Product ID ${product.id} is live in the storefront. Finish the detail-page content here.`;
	}
	...
	[visibilitySelect, prioritySelect, orderIndexInput, highlightTagSelect].forEach((field) => {
